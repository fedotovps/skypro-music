...
describe("Корректный рендер компонента Player", () => {
    ...
    it("Переключение следующего трека", () => {
        ...
        const nextTrackClick = component.root.findByProps({"data-testid": "next-track-button"});
        act(() => {
        ...
        const setCurrentTrackAction = actions.find(action => action.type === setCurrentTrack.type);
